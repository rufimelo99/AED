...
int tsp_v2(int initial_node, int amount_of_cities, int mask[amount_of_cities]){
  ...
  if(n_opcoes==0){
    ...
    int mascara=conv_array_bin(amount_of_cities, mask);
    mask_plus_city[mascara][initial_node]=matriz[initial_node][0];
   
    if DEBUG {
      printf("mascara: %i\n", mascara);
      printf("initial node %i\n", initial_node);
      printf("valor q/ opcao 0 %i\n", mask_plus_city[mascara][initial_node]);
    ...
    for(int i=0; i<n_opcoes; i++){  
      ...
      mask_plus_city[mask_temp_int][cidades_em_falta[i]]=matriz[cidades_em_falta[i]][initial_node]+tsp_v2(cidades_em_falta[i], amount_of_cities, mask_temp);
      int valor = mask_plus_city[mask_temp_int][cidades_em_falta[i]];
      //ERROR
...
//Errado     
  int tsp_v3(int amount_of_cities, int initial_node){
    int minimo=999999; //numero excesivamente grande
    //TODO 
    printf("\nNumero de cidades: %i\n", amount_of_cities);

    //criar mascara apenas com o primeiro elemento ocupado
    ...
    //
    int mask_original_int=conv_array_bin(amount_of_cities, mask);
    //printf("mask_int %i\n", mask_original_int);
    
    
    if(amount_of_cities==2){
      //distancia de ir e voltar
      minimo=matriz[0][1]+matriz[1][0];
      mask[1]=1;

      int mask_int=conv_array_bin(amount_of_cities, mask);
      mask_plus_city[mask_int][1]=minimo;
    }
    if(amount_of_cities==3){
      ...
      int n_opcoes=0;
        ...
        //WORKING HERE
        //      armazenar
        int parcela=9999999; //numero excesivamente grande
        for(int i=0; i<n_opcoes; i++){
          int mask_temp[amount_of_cities];
          for(int j=0; j<amount_of_cities; j++){
            mask_temp[j]=mask[j];
          }
          //ocupar na mask a posicao da cidade a considerar
          mask_temp[cidades_em_falta[i]]=1;
          //para primeiro ciclo ---> cidades_em_falta[i]=1
          //
          if DEBUG{
            for(int i=0; i<amount_of_cities; i++){  
          ...
          //desnecessário
          if(i==0){
              //i--> primeira cidade a verificar
              //i+1--> segunda cidade a verfificar
              printf("matriz[cidades_em_falta[i]][cidades_em_falta[i+1]]: %i\n", matriz[cidades_em_falta[i]][cidades_em_falta[i+1]]);
              printf("mask_plus_city[mask_cheia_int][i]: %i\n", mask_plus_city[mask_cheia_int][cidades_em_falta[i]]);
              parcela= matriz[cidades_em_falta[i+1]][cidades_em_falta[i]]+mask_plus_city[mask_cheia_int][i];
              printf("parcela: %i\n", parcela);
          }
          */
          if(i==1){
              //i--> primeira cidade a verificar
              //i-1--> segunda cidade a verfificar
              parcela= matriz[cidades_em_falta[i]][cidades_em_falta[i-1]]+mask_plus_city[mask_cheia_int][cidades_em_falta[i]];
              if DEBUG{
                printf("matriz[cidades_em_falta[i-1]][cidades_em_falta[i]]: %i\n", matriz[cidades_em_falta[i-1]][cidades_em_falta[i]]);
                printf("mask_plus_city[mask_cheia_int][i]: %i\n", mask_plus_city[mask_cheia_int][cidades_em_falta[i-1]]);
                printf("parcela: %i\n", parcela);
              }
          }
          if(parcela<minimo){
          ...
          mask_plus_city[mask_original_int][initial_node]=minimo;
          
        }

          //for i<quantidade_cidades_q_faltam
          //   save mask_plus_city[nova mask][cidade_q_faltam[i]]= matriz[initial_node????][cidade_q_faltam[i]]
          //                                                     + mask_plus_city[mask_antiga][cidade_q_faltam[i]]
          //
          //      marcar nas mascaras
          //      armazenar
          //                                                             que cidade? supostamente ultima a ser adicionada
          //                                                             mask antiga--> (com a possicao da cidade que falta a 0)
          //                                                             mask nova--> (com a possicao da cidade que se usa a 1(cidade_q_faltam[i]))
          //arranjar minimo + mask
          ...
          int mask_temp_int=conv_array_bin(amount_of_cities, mask_temp);
          mask_plus_city[mask_temp_int][1]=minimo;
          */
      }
      ...
      //criar a mascara em que esteja tudo 1, excepto as cidades que faltam
      /*
        int mask_temp[amount_of_cities];
        for(int j=0; j<amount_of_cities; j++){
            mask_temp[j]=1;
        ...
        for(int i=0; i<n_opcoes; i++){
          mask_temp[cidades_em_falta[i]]=0;
        }
      */
      ...
      for(int i=0; i<n_opcoes; i++){
        ...
        //EDIT HERE
        //ja armazenado anteriormente
        if(mask_plus_city[mask_temp_int][cidades_em_falta[i]]!=-1){
          continue;
        }
        else{
          
          tsp_v3(amount_of_cities-1, i);
        }
        parcela=matriz[cidades_em_falta[i]][initial_node]+mask_plus_city[mask_temp_int][cidades_em_falta[i]];
...
//
int tsp_v4(int initial_node, int amount_of_cities, int mask[amount_of_cities]){
  int minimo=999999; //numero excesivamente grande
  //TODO 
  //ocupar na mascara o node usado
    mask[initial_node]=1;
    ...
    if(n_opcoes==0){
      ...
      for(int i=0; i<n_opcoes;i++){
        ...
        if(mask_plus_city[mask_temp_int][cidades_em_falta[i]]==0){
          ...
          tsp_v4(cidades_em_falta[i], amount_of_cities, mask_temp);
        }
        else{ 
          if DEBUG {
            ...
            printf("parcela neste instante: %i\n", parcela);
          }
          continue;
        }
...
int main(int argc,char **argv)
{
  ...
  */
  ...
  //inicalizar a estrutura bidimensional
    
    /*
    for(int i=0; i<3000000; i++){
      for(int j=0; i<18; i++){
        //mask_plus_city[300000][18]
        mask_plus_city[i][j]=-1;
        printf("mask_plus_city[i][j]: %i\n", mask_plus_city[i][j]);
      }
    } 
    
    int mask_cheia[n_cities];
    for(int i=0; i<n_cities;i++){
      mask_cheia[i]=1;
    }
    if DEBUG{
      for(int i=0; i<n_cities;i++){
        printf("%i ", mask_cheia[i]);
      }

      printf("\n"); 
    }
    mask_cheia_int=conv_array_bin(n_cities, mask_cheia);
    */
    //imprime_exemplo_matriz(); 
    /*
    for(int i=0; i<n_cities; i++){
      mask_plus_city[mask_cheia_int][i]=matriz[i][0];
      if DEBUG{
        printf("elemento na estrutura: %i\n", mask_plus_city[mask_cheia_int][i]);
      }
    }
    */
  //
  ...
  for(int i=2; i<=4; i++){
    ...
    int minimo = tsp_v3(amount_of_cities, 0);
    printf("\nMinimo: %i\n", minimo);
  }
  */
  ...
  for(int i=3; i<=18; i++){  
    ...
    int resultado=tsp_v4(0, amount_of_cities , mascara_exemplo);
    printf("resultado para %i cidades: %i\n\n\n", amount_of_cities, resultado);
